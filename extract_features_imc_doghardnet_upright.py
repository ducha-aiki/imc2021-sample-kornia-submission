...
def extract_features(img_fname, detector, descriptor, device, visualize=False):
    ...
    if True:
        ...
        kpts = np.array(unique_kp)[idxs[:min(len(unique_kp), 8000)]]
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        "--save_path",
        type=str,
        default=os.path.join('extracted', 'cv2-dog-uprighthardnet'),
    ...
    try:
        ...
        print ('CPU mode')
    # SIFT (DoG) Detector
    sift_det =  cv2.SIFT_create(12000, contrastThreshold=-10000, edgeThreshold=-10000)
    # HardNet descriptor
