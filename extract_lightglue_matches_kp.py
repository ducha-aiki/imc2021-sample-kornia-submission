...
def get_data(kps,  descs,img_key):
    kp1 = kps[img_key]
    descs1 = descs[img_key]
    return kp1,  descs1


def match_lightglue_with_degensac(kp1, kp2, descs1, descs2,
                               h1, w1, h2, w2, ds_name='phototourism'):
    d1 = torch.from_numpy(descs1).to(device)
    d2 = torch.from_numpy(descs2).to(device)
    laf1 = KF.laf_from_center_scale_ori(torch.from_numpy(kp1[None]).to(device))
    laf2 = KF.laf_from_center_scale_ori(torch.from_numpy(kp2[None]).to(device))
    with torch.inference_mode():
        dists, idxs = lg_matcher(torch.from_numpy(descs1).to(device),
                         torch.from_numpy(descs2).to(device), laf1, laf2,
                         hw1=(h1,w1), hw2=(h2,w2))
    #print (idxs.shape)
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        "--feature", type=str, default='disk')
    parser.add_argument(
        "--save_path",
        type=str,
        default=os.path.join('extracted', 'disk'),
        ...
        for seq in seqs[::-1]:
            ...
            with h5py.File(f'{seq_out_path}/matches.h5', 'w') as f_m:
                for i1, img1_fname in tqdm(enumerate(img_fnames)):
                    ...
                    w1, h1 = img1.size
                    kp1, descs1 = get_data(kps, descs, img1_key)
                    for img2_fname in tqdm(img_fnames[i1+1:]):
                        ...
                        kp2, descs2 = get_data(kps, descs,  img2_key)
                        _, idxs = match_lightglue_with_degensac(kp1, kp2, descs1, descs2, h1, w1, h2, w2, ds_name=ds)
